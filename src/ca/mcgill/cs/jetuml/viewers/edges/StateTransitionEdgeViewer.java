...
public final class StateTransitionEdgeViewer extends AbstractEdgeViewer {
    ...
    private void drawArrowHead(Edge pEdge, GraphicsContext pGraphics) {
        if (isSelfEdge(pEdge)) {
            ...
            if (getPosition(pEdge) == 1) {
                ArrowHead.V.view().draw(pGraphics, new Point(connectionPoint2.getX() + SELF_EDGE_OFFSET, connectionPoint2.getY() - SELF_EDGE_OFFSET / 4), getConnectionPoints(pEdge).getPoint2());
            } else {
                ArrowHead.V.view().draw(pGraphics, new Point(connectionPoint2.getX() - SELF_EDGE_OFFSET / 4, connectionPoint2.getY() - SELF_EDGE_OFFSET), getConnectionPoints(pEdge).getPoint2());
            }
        } else {
    ...
    private void drawLabel(StateTransitionEdge pEdge, GraphicsContext pGraphics) {
        ...
        Rectangle drawingRectangle = new Rectangle((int) Math.round(labelBounds.getMinX()), (int) Math.round(labelBounds.getMinY()), (int) Math.round(labelBounds.getWidth()), (int) Math.round(labelBounds.getHeight()));
    ...
    private void drawSelfEdge(Edge pEdge, GraphicsContext pGraphics) {
        ...
        pGraphics.strokeArc(arc.getCenterX(), arc.getCenterY(), arc.getRadiusX(), arc.getRadiusY(), arc.getStartAngle(), arc.getLength(), arc.getType());
     ...
     * @dscribe Returns Dimension.NULL when input is null.
     */
    ...
    private Rectangle2D getNormalEdgeLabelBounds(StateTransitionEdge pEdge) {
        ...
        int gap = 3;
        if (line.getY1() >= line.getY2() - VERTICAL_TOLERANCE && line.getY1() <= line.getY2() + VERTICAL_TOLERANCE) {
    ...
    private Rectangle2D getSelfEdgeLabelBounds(StateTransitionEdge pEdge) {
        ...
        Dimension textDimensions = getLabelBounds(label);
        if (getPosition(pEdge) == 1) {
            return new Rectangle2D(line.getX1() + SELF_EDGE_OFFSET - textDimensions.width() / 2, line.getY1() - SELF_EDGE_OFFSET - textDimensions.height(), textDimensions.width(), textDimensions.height());
        } else {
            return new Rectangle2D(line.getX1() - textDimensions.width() / 2, line.getY1() - 2 * SELF_EDGE_OFFSET - textDimensions.height(), textDimensions.width(), textDimensions.height());
        }
    }
    ...
    private String wrapLabel(StateTransitionEdge pEdge) {
        int distanceInX = Math.abs(NodeViewerRegistry.getBounds(pEdge.getStart()).getCenter().getX() - NodeViewerRegistry.getBounds(pEdge.getEnd()).getCenter().getX());
        int distanceInY = Math.abs(NodeViewerRegistry.getBounds(pEdge.getStart()).getCenter().getY() - NodeViewerRegistry.getBounds(pEdge.getEnd()).getCenter().getY());
    ...
    private Line getSelfEdgeConnectionPoints(Edge pEdge) {
        if (getPosition(pEdge) == 1) {
            Point2D point1 = new Point2D(NodeViewerRegistry.getBounds(pEdge.getStart()).getMaxX() - SELF_EDGE_OFFSET, NodeViewerRegistry.getBounds(pEdge.getStart()).getY());
            Point2D point2 = new Point2D(NodeViewerRegistry.getBounds(pEdge.getStart()).getMaxX(), NodeViewerRegistry.getBounds(pEdge.getStart()).getY() + SELF_EDGE_OFFSET);
            return new Line(Conversions.toPoint(point1), Conversions.toPoint(point2));
        } else {
            Point2D point1 = new Point2D(NodeViewerRegistry.getBounds(pEdge.getStart()).getX(), NodeViewerRegistry.getBounds(pEdge.getStart()).getY() + SELF_EDGE_OFFSET);
            Point2D point2 = new Point2D(NodeViewerRegistry.getBounds(pEdge.getStart()).getX() + SELF_EDGE_OFFSET, NodeViewerRegistry.getBounds(pEdge.getStart()).getY());
            return new Line(Conversions.toPoint(point1), Conversions.toPoint(point2));
        }
    }
    ...
    private Shape getNormalEdgeShape(Edge pEdge) {
        ...
        QuadCurveTo curveTo = new QuadCurveTo(getControlPoint(pEdge).getX(), getControlPoint(pEdge).getY(), line.getPoint2().getX(), line.getPoint2().getY());
