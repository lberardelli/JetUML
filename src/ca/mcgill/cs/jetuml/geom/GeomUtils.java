...
/**
 * A collection of utility methods related to geometric calculations and supporting functions.
 */
public final class GeomUtils
{
	...
	/**
	 * Returns the point that intersects the line that runs in direction pDirection from the center of pRectangle, with
	 * one side of pRectangle.
	 * 
	 ...
	 * @param pDirection
	 *            The direction to follow to intersect the sides of the rectangle.
	 * @return The point that intersects pRectangle if we draw a line from its center going in direction pDirection.
	...
	public static Point intersectRectangle(Rectangle pRectangle, Direction pDirection)
	{
		...
		Direction diagonalNE = Direction.fromLine(pRectangle.getCenter(),
				new Point(pRectangle.getMaxX(), pRectangle.getY()));
		Direction diagonalSE = Direction.fromLine(pRectangle.getCenter(),
		...
		{
			int offset = lengthOfOpposingSide(pDirection.asAngle() - Direction.EAST.asAngle(),
					pRectangle.getWidth() / 2);
			return new Point(pRectangle.getMaxX(), pRectangle.getCenter().getY() + offset);
		}
		else if (pDirection.isBetween(diagonalSE, diagonalSW))
		{
			int offset = lengthOfOpposingSide(pDirection.asAngle() - Direction.SOUTH.asAngle(),
					pRectangle.getHeight() / 2);
			return new Point(pRectangle.getCenter().getX() - offset, pRectangle.getMaxY());
		}
		else if (pDirection.isBetween(diagonalSW, diagonalNW))
		{
			int offset = lengthOfOpposingSide(pDirection.asAngle() - Direction.WEST.asAngle(),
					pRectangle.getWidth() / 2);
	...
	/**
	 * Returns the point that intersects a circle on the line that originates at the center of pBounds going in
	 * direction pDirection.
	 * 
	 * @param pBounds
	 *            The bounds of the rectangle in which the circle to intersect in inscribed.
	 * @param pDirection
	 *            The direction to follow to intersect the sides of the circle.
	 * @return The point that intersects the circle inscribed in pBounds if we draw a line from its center going in
	 *         direction pDirection.
	...
	/**
	 * Returns the point that intersects a ellipse on the line that originates at the center of pBounds going in
	 * direction pDirection.
	 * 
	 * @param pBounds
	 *            The bounds of the rectangle in which the ellipse to intersect in inscribed.
	 * @param pDirection
	 *            The direction to follow to intersect the sides of the ellipse.
	 * @return The point that intersects the ellipse inscribed in pBounds if we draw a line from its center going in
	 *         direction pDirection.
	...
	/**
	 * @param pBounds
	 *            The bounds of the rectangle in which the rounded rectangle to intersect in inscribed.
	 * @param pDirection
	 *            The direction to follow to intersect the sides of the rounded rectangle.
	 * @return The point that intersects the rounded rectangle inscribed in pBounds if we draw a line from its center
	 *         going in direction pDirection.
	 * @pre pBounds != null && pDirection != null
	 */
	public static Point intersectRoundedRectangle(Rectangle pBounds, Direction pDirection)
	{
		...
		Direction topNE = Direction.fromLine(pBounds.getCenter(),
				new Point(pBounds.getMaxX() - radius, pBounds.getY()));
		Direction bottomNE = Direction.fromLine(pBounds.getCenter(),
				new Point(pBounds.getMaxX(), pBounds.getY() + radius));
		Direction topSE = Direction.fromLine(pBounds.getCenter(),
				new Point(pBounds.getMaxX(), pBounds.getMaxY() - radius));
		Direction bottomSE = Direction.fromLine(pBounds.getCenter(),
				new Point(pBounds.getMaxX() - radius, pBounds.getMaxY()));
		...
		{
			Point cornerCenter = new Point(pBounds.getCenter().getX() + widthOffset,
					pBounds.getCenter().getY() - heightOffset);
			...
			result = new Point(pBounds.getCenter().getX() + offsetX + widthOffset,
					pBounds.getCenter().getY() + offsetY - heightOffset);
		}
	...
	/*
	 * returns the length, in pixel, of the opposing side to the angle pAngleInDegrees of a right triangle for when the
	 * length (in pixels) of the adjacent side is known.
	 */
