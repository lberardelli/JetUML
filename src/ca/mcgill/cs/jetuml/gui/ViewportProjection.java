...
/**
 * An instance of this class can be used to make viewport projection computations to compare visible areas of a
 * scrollpane viewport with an underlying canvas. Instances of this class are intended to be used in a narrow scope: to
 * be initialized, used to perform calculations, then discarded. Both dimensions of the viewport must be equal to or
...
public final class ViewportProjection
{
	 ...
	 *
	 * @param pViewportWidth
	 *            The width of the viewport.
	 * @param pViewportHeight
	 ...
	 * @param pHValue
	 *            The horizontal position of the viewport over the canvas, from 0 to 1.
	 * @param pVValue
	 *            The vertical position of the viewport over the canvas from 0 to 1.
	 */
	public ViewportProjection(int pViewportWidth, int pViewportHeight, int pCanvasWidth, int pCanvasHeight,
			double pHValue, double pVValue)
	{
	...
	/**
	 * @return The ratio (in the unit interval) of the canvas width covered by the viewport.
	 */
	...
	/**
	 * @return The ratio (in the unit interval) of the canvas height covered by the viewport.
	 */
	...
	/**
	 * @return The number of pixels on the left of the canvas that are not visible in the viewport.
	 */
	...
	/**
	 * @return The number of pixels on the right of the canvas that are not visible in the viewport.
	 */
	...
	/**
	 * @return The number of pixels at the top of the canvas that are not visible in the viewport.
	 */
	...
	/**
	 * @return The number of pixels at the bottom of the canvas that are not visible in the viewport.
	 */
	...
	/**
	 * @param pX
	 *            A x-coordinate on the canvas.
	 * @return True if the coordinate is hidden on the left of the viewport.
	 */
	public boolean isHiddenLeft(int pX)
	{
		assert pX >= 0 && pX <= aCanvasWidth : "Argument expected to be between 0 and " + aCanvasWidth + " but was "
				+ pX;
	...
	/**
	 * @param pX
	 *            An x-coordinate to reveal.
	 * @return A newly computed HValue that will ensure pX is just visible in the viewport.
	 */
	...
	/**
	 * @param pY
	 *            A y-coordinate to reveal.
	 * @return A newly computed VValue that will ensure pY is just visible in the viewport.
	 * @dscribe When hiddenTop returns correctly
	 * @dscribe When hiddenBottom returns correctly
	 */
	@StatefulReturnsDouble(factory = "new ViewportProjection(8,8,10,10,.5,.5)", params = "1", target = "-3", state = "hiddenBottom")
	@StatefulReturnsDouble(factory = "new ViewportProjection(8,8,10,10,.5,.5)", params = {
			"9" }, target = "4.5", state = "hiddenTop")
	...
	{
		return String.format("[ViewportProjection: vp=%dx%d; canvas=%dx%d; pos=%.2f, %.2f]", aViewportWidth,
				aViewportHeight, aCanvasWidth, aCanvasHeight, aHValue, aVValue);
	}
}
