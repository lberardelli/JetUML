...
public class TestDiagram {
    ...
    private static Stream<Arguments> argumentsForFileExtensions() {
        return Stream.of(Arguments.of(new Diagram(DiagramType.CLASS), ".class"), Arguments.of(new Diagram(DiagramType.SEQUENCE), ".sequence"), Arguments.of(new Diagram(DiagramType.STATE), ".state"), Arguments.of(new Diagram(DiagramType.OBJECT), ".object"), Arguments.of(new Diagram(DiagramType.USECASE), ".usecase"));
    }
    ...
    @Test
    @AssertBoolFieldManip(bool = "False", factory = "aDiagram", fieldName = "aDiagram", params = { "aNode2" }, state = "DoesNotContainNode", statement = "aDiagram.addRootNode(aNode2)", uut = "contains(DiagramElement)")
    public void contains_WhenDoesNotContainNode_ReturnFalse() {
        aDiagram.addRootNode(aNode2);
        boolean actual = aDiagram.contains(aNode2);
        assertFalse(actual);
    }

    @Test
    @AssertBoolFieldManip(bool = "True", factory = "aDiagram", fieldName = "aDiagram", params = { "aNode1" }, state = "ContainsElement", statement = "aDiagram.addRootNode(aNode1)", uut = "contains(DiagramElement)")
    public void contains_WhenContainsElement_ReturnTrue() {
        aDiagram.addRootNode(aNode1);
        boolean actual = aDiagram.contains(aNode1);
        assertTrue(actual);
    }
}
